...
export const allProductsQuery = `
  *[_type == "saree" && ${publishedOnly} && coalesce(status, "active") == "active"] | order(coalesce(sortOrder, 100) asc, _createdAt desc) ${productCardFields}
`;

/** Fetch a single product by slug — full detail for PDP */
export const productBySlugQuery = `
  *[_type == "saree" && ${publishedOnly} && slug.current == $slug && coalesce(status, "active") == "active"][0] {
    _id,
    _type,
    _createdAt,
    _updatedAt,
